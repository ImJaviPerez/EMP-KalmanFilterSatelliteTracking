...
const uint16_t COLOR_WHITE_SMOKED = 0xF7BE;
const uint16_t COLOR_BLUE_SYSTEM = 0X3B32;
const uint16_t COLOR_BLUE = 0X00FF;
const uint16_t COLOR_GREEN = 0X47E0;
const uint16_t COLOR_DARKGREEN = 0X0FE0;
...
public:
  ...
  void SetReset(void);
	...
	void SetLabelValueInt(uint8_t page_id,uint8_t control_id,short number);
	void SetValueInt(uint8_t page_id,uint8_t sign_control_id,uint8_t number_control_id,short number);
	void SetBarGraph(uint16_t xLcd,uint16_t yLcd, uint16_t barWidth,uint16_t barHeight, uint16_t barPercentage, uint16_t ForeColor);
