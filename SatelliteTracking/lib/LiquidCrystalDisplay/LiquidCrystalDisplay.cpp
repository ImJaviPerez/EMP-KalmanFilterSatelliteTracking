...
void LiquidCrystal::TFTInit(uint8_t rst)
{
  ...
  digitalWrite(_rst_pin,LO_VAL);//Reset LCDs
  delayMicroseconds(1000);
  digitalWrite(_rst_pin,HI_VAL);
...
/**
 * @brief The member function SetNumberValue(...) writes only positive numbers.
 * 
 * @param page_id 
 * @param control_id 
 * @param number 
...
/**
 * @brief This new member function writes positive and negative numbers
 *  but it needs a large space, larger than the number. So, it could overlap
 * other fields in the LCD.
 * 
 * @author imjaviperez
 * @param page_id 
 * @param control_id 
 * @param number 
 */
void LiquidCrystal::SetLabelValueInt(uint8_t page_id,uint8_t control_id,short number)
{
	sendbuf[0] = LABLE;
	sendbuf[1] = control_id;
	sendbuf[2] = page_id;
	...
	char strInt[5];
	itoa(number,strInt,10);

	uint8_t i=0;
	while(strInt[i] != 0)
	{									
		sendbuf[i+3] = strInt[i];
		i++;
	}
	I2C_Send(sendbuf,3+i);
}
...
/**
 * @brief Paints number's sign in a LABLE object 
 * and paints the number's absolute value in a NUMBER object.
 * But  it needs a large space, larger than the number. 
 * So, it could overlap other fields in the LCD.
 * 
 * @author imjaviperez
 * @param page_id 
 * @param sign_control_id 
 * @param number_control_id 
 * @param number 
 */
void LiquidCrystal::SetValueInt(uint8_t page_id,uint8_t sign_control_id,uint8_t number_control_id,short number)
{
	sendbuf[0] = LABLE;
	sendbuf[1] = sign_control_id;
	sendbuf[2] = page_id;
	...
	sendbuf[3] = number_sign;
	I2C_Send(sendbuf,4);
	// Set number
	SetNumberValue(page_id,number_control_id, number);
}
 ...
 * @param ForeColor 
 */
void LiquidCrystal::SetBarGraph(uint16_t xLcd,uint16_t yLcd, uint16_t barWidth,uint16_t barHeight, uint16_t barPercentage, uint16_t ForeColor)
{
	// Top bar
	uint16_t topBarHeight = round(barHeight * (100 - barPercentage)/100);
	if (topBarHeight > 0)
	{
		RectangleFill(xLcd,yLcd,barWidth,topBarHeight,COLOR_WHITE_SMOKED);
	}

	// Bottom bar
	if (barPercentage != 0)
	{
		uint16_t yLcdBottomBar = yLcd + topBarHeight + 1;
		uint16_t bottomBarHeight = barHeight - topBarHeight;
		RectangleFill(xLcd,yLcdBottomBar,barWidth,bottomBarHeight,ForeColor);
	}
